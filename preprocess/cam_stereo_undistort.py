...
def undistort_for_date(date, data_dir='D:\\RawData'):
    ...
    calib_yaml_l = os.path.join(data_dir, 'calib', '18384019-20190924', 'ost.yaml')
    calib_yaml_r = os.path.join(data_dir, 'calib', '19325055-20190924', 'ost.yaml')
    seqs = sorted(os.listdir(base_dir))
    for seq in seqs:
        folder_dir_l = os.path.join(base_dir, seq, 'images_0')
        print("Undistorting %s ..." % folder_dir_l)
        undistort_for_seq(folder_dir_l, calib_yaml_l)
        folder_dir_r = os.path.join(base_dir, seq, 'images_1')
        print("Undistorting %s ..." % folder_dir_r)
        undistort_for_seq(folder_dir_r, calib_yaml_r)

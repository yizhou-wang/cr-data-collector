                        ...
                        # if timeSinceStart <= self.duration:
                            ...
                            if select.select([self.d_socket], [], [], 0)[0]:
                                ...
                                if not self.lidar_only:
                                    if self.pps_status == 0 and flag == 0:
                                ...
                                if self.pps_status == 0 and flag == 1 and self.lidar_only == False:
                    ...
                    if self.firmwareType == 1:
                        ...
                        if self.dataType == 2:
                            ...
                            for i in range(0, lenData):
                                ...
                                numPts += 1
                                csvFile.write("5,1,1,0,0x00000000,0,2," + str(timestamps[i]) \
                                              + "," + "{0:.3f}".format(coord1) \
                                              + "," + "{0:.3f}".format(coord2) \
                                              + "," + "{0:.3f}".format(coord3) \
                        ...
                        if time.time() - last_time < self.duration:
                            ...
                            if select.select([self.i_socket], [], [], 0)[0]:
                                ...
                                if dataType == 6:
                                    ...
                                    IMU_file.write(struct.pack('<d', time.time()))
                                    imu_records += 1
...
class openpylivox(object):
    ...
    def dataStart(self):
        ...
        if self._isConnected:
            if not self._isData:
                self._captureStream = _dataCaptureThread(self._sensorIP, self._dataSocket, "", 2, 0, 0, 0,
    ...
    def saveDataToCSV(self, filePathAndName, secsToWait, duration, lidar_only):
                            ...
                            if secsToWait < 0:
                                ...
                                if secsToWait > 900:
                                    ...
                                    if filePathAndName == "":
                                        ...
                                        self._captureStream.lidar_only = lidar_only
                                        time.sleep(0.1)
    ...
    def saveDataToFile(self, filePathAndName, secsToWait, duration, lidar_only):
        path_file = Path(filePathAndName)
        filename = path_file.stem
        exten = path_file.suffix
        self._saveDataToFile(filePathAndName, secsToWait, duration, lidar_only)
        for i in range(len(self._mid100_sensors)):
            new_file = ""
            if i == 0:
            ...
            self._mid100_sensors[i]._saveDataToFile(new_file, secsToWait, duration, lidar_only)
